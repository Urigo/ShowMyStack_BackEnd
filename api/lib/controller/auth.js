...
exports.login = {
  ...
  notes:
    'Response for a bad request: <br/>' +
    '<pre><code>' + JSON.stringify({"status":"error","message":"Wrong credentials"}, null, 4) + '</code></pre>' +
    '<br/><br/>' +
    'Valid response:' +
    '<pre><code>' +
    JSON.stringify({"user":{"__v":1,"active":true,"createdAt":"2014-03-04T09:25:23.191Z","email":"test","role":"EDITOR","updatedAt":"2014-03-04T09:26:09.156Z","verified":false},"access_token":"JPWWx8CYx3FKcJ1cFIos9GfQAmiSKnSu"}, null, 4) +
    '</code></pre>'
  ,
  validate: {
    payload: {
  ...
  handler: function (request, reply) {
    this.passport.authenticate('local', function(err, user, info) {
      if (err) { return reply({status: "error", message: err}) }
      if (!user) { return reply({status: "error", message: "Wrong credentials"}).code(401); }

      // Generate the token if needed and return it
      if (user.access_tokens.length === 0) {
        user.generateToken(null, function(err, token) {
          user.save(function(err,response) {
            if (err) return reply({status: 'error', message: err}).code(500);
            reply({user: user.toObject(), access_token: token.token});
          });
        });
      } else {
        reply({user: user.toObject(), access_token: user.access_tokens[0].token});
      }
...
exports.register = {
  ...
  notes: 'Valid response: <br/>' +
    '<pre><code>' +
    JSON.stringify({"__v":0,"createdAt":"2014-03-25T23:15:49.001Z","updatedAt":"2014-03-25T23:15:49.001Z","email":"test","role":"EDITOR","active":true,"verified":false}, null, 4) +
    '</code></pre>',
  // Validation
  validate: {
    payload: {
...
exports.fbRegister = {
  ...
  notes: 'Valid response: <br/>' +
    '<pre><code>' +
    JSON.stringify({"_id":"5351a21cbf4dd7aee3b28aad","email":null,"name":"Open Graph Test User","role":"EDITOR","active":true,"verified":false,"facebook":{"id":"100007773222474"}}, null, 4) +
    '</code></pre>',
  validate: {
    payload: {
  ...
  handler: function(request, reply) {
    ...
    FB.api('/me', {fields: ['id', 'email', 'name', 'birthday', 'gender']}, function(res) {
      if(!res || res.error) {
...
exports.fbLogin = {
  ...
  notes:
    'Response for a bad request: <br/>' +
    '<pre><code>' + JSON.stringify({"status":"error","message":"Unknown user"}, null, 4) + '</code></pre>' +
    '<br/><br/>' +
    'Valid response: <br/>' +
    '<pre><code>' +
    JSON.stringify({"user":{"__v":1,"createdAt":"2014-04-18T22:45:39.873Z","updatedAt":"2014-04-18T22:45:39.873Z","_id":"5351ab11bf4dd7aee3b28ab9","email":null,"name":"Open Graph Test User","role":"EDITOR","active":true,"verified":false,"facebook":{"id":"100007773222474"}},"access_token":"53zNKcb0UkJFAyD6cnkLCGhBPnC1Fsto"}, null, 4) +
    '</code></pre>',
  validate: {
    payload: {
      accessToken: Joi.string().required()
  ...
  handler: function(request, reply) {
    ...
    FB.api('/me', {}, function(res) {
      ...
      UserCollection.findByFacebook(res.id, function(err, user) {
        if (err || !user) return reply({status: "error", message: "Unknown user"}).code(401);
